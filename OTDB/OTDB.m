...
- (instancetype)init {
    if (self = [super init]) {
        otDbQueue = [FMDatabaseQueue databaseQueueWithPath:[OTDB dbFilePath]];
    ...
    if (modelArr.count > 0) {
        ...
        if (cmpResult != TZSDbTableTheSame) {
            [otDbQueue inDatabase:^(FMDatabase *db) {
                @try {
                    if (![db open]) {
                        KCLog(@"%@",KCLocalizedStr(@"DB_ERROR"));
                        return ;
                    }
                    NSString *createSQL = [OTDB createTableSQLWithModel:(NSObject *)model inDb:db];
                    db.shouldCacheStatements = YES;
                    if (![db executeUpdate:createSQL]) {
                        KCLog(@"create DB fail - %@", createSQL);
                    };
                }
                @catch (NSException *exception) {
                    KCLog(@"%@%@",KCLocalizedStr(@"DB_EXCEPTION"),exception.userInfo.description);
                }
                @finally {
                    [db close];
                }
    ...
    for (NSObject * delModel in arrOfmodel) {
        ...
        [otDbQueue inDatabase:^(FMDatabase *db) {
            @try {
                ...
                KCLog(@"executing insert sql - %@",sqlString);
...
{
    if ([model isKindOfClass:[OTDBTableVersion class]])
    ...
    if ([model.class conformsToProtocol:@protocol(OTDBMigrationProtocol)]) {
...
{
    NSString * tableName = NSStringFromClass([OTDBTableVersion class]);
    if (![self isTableExist:tableName]) {
        [otDbQueue inDatabase:^(FMDatabase *db) {
            @try {
                ...
                NSString *createSQL = [OTDB createTableSQLWithModel:(NSObject *)[[OTDBTableVersion alloc]init] inDb:db];
                db.shouldCacheStatements = YES;
                if (![db executeUpdate:createSQL]) {
                    KCLog(@"create DB fail - %@", createSQL);
                };
...
{
    OTDBTableVersion * tVersion = [[OTDBTableVersion alloc]init];
...
{
    NSString * tableName = NSStringFromClass(modelClass);
    NSArray * tVersionArray = [self queryWithClass:[OTDBTableVersion class] key:@"tablename" value:tableName orderByKey:nil desc:NO];
    if (tVersionArray != nil && [tVersionArray count] == 1) {
        OTDBTableVersion * tVersion = tVersionArray.lastObject;
...
- (BOOL) migrateClassTable: (NSObject *) model
{
    ...
    [otDbQueue inDatabase:^(FMDatabase *db) {
        @try {
            ...
            if (![db executeUpdate:createTableSQL]) {
                KCLog(@"create DB fail - %@", createTableSQL);
            };
        }
        @catch (NSException *exception) {
            KCLog(@"%@%@",KCLocalizedStr(@"DB_EXCEPTION"),exception.userInfo.description);
    ...
    OTDBTableVersion * newDBTVersion = [[OTDBTableVersion alloc]init];
...
- (void) dumpObject: (NSObject *) obj
{
    ...
    if ( [obj conformsToProtocol:@protocol(OTDBMigrationProtocol)]) {
